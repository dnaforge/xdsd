...
class LoopGraphicsItem(QGraphicsItem):
    ...
    def __init__(self, loop, start, end, prev, next):
        ...
        center = [(self.end[0] + self.start[0]) / 2, (self.end[1] + self.start[1]) / 2]
        ...
        # LOOP FLIPPING
        normal = [
            chord_direction[0] / get_vector_length(chord_direction),
            chord_direction[1] / get_vector_length(chord_direction),
        ]

        prev_domain_direction = [self.start[0] - prev[0], self.start[1] - prev[1]]
        ...
        first_reflection = [
            -first_loop_domain_direction[0]
            - 2
            * dot(
                [-first_loop_domain_direction[0], -first_loop_domain_direction[1]],
                normal,
            )
            * normal[0],
            -first_loop_domain_direction[1]
            - 2
            * dot(
                [-first_loop_domain_direction[0], -first_loop_domain_direction[1]],
                normal,
        ...
        next_domain_direction = [self.end[0] - next[0], self.end[1] - next[1]]
        last_loop_domain_direction = [
            self.loop_center[0] - self.end[0],
            self.loop_center[1] - self.end[1],
        ]
        last_reflection = [
            -last_loop_domain_direction[0]
            - 2
            * dot(
                [-last_loop_domain_direction[0], -last_loop_domain_direction[1]], normal
            )
            * normal[0],
            -last_loop_domain_direction[1]
            - 2
            * dot(
                [-last_loop_domain_direction[0], -last_loop_domain_direction[1]], normal
            )
        ...
        first_angle = abs(
            get_angle_negative(prev_domain_direction, first_loop_domain_direction)
        )
        first_angle_reflection = abs(
            get_angle_negative(prev_domain_direction, first_reflection)
        )

        last_angle = abs(
            get_angle_negative(next_domain_direction, last_loop_domain_direction)
        )
        last_angle_reflection = abs(
            get_angle_negative(next_domain_direction, last_reflection)
        )

        if (
            first_angle - first_angle_reflection + last_angle - last_angle_reflection
            > 0
        ):
            self.flip = True
        else:
        ...
        vector = [
            vector[0] * offset / vector_length,
            vector[1] * offset / vector_length,
        ]
        self.loop_center = [
            center[0] + factor * vector[0],
            center[1] + factor * vector[1],
        ]

        if self.flip is False:
            center_to_start = [
                self.start[0] - self.loop_center[0],
                self.start[1] - self.loop_center[1],
            ]
            center_to_end = [
                self.end[0] - self.loop_center[0],
                self.end[1] - self.loop_center[1],
            ]
        else:
            center_to_start = [
                self.end[0] - self.loop_center[0],
    ...
    def boundingRect(self):
        margin = 20
        return QRectF(
            self.loop_center[0] - self.r - margin,
            self.loop_center[1] - self.r - margin,
            2 * self.r + 2 * margin,
            2 * self.r + 2 * margin,
        )
    ...
    def paint(self, painter, option, widget):
        ...
        rect = QRectF(
            self.loop_center[0] - self.r,
            self.loop_center[1] - self.r,
            2 * self.r,
            2 * self.r,
        )
        path = QPainterPath()
        ...
        for domain in self.loop.domains:
            ...
            text_vector = [
                (prev_position.x() + curr_position.x()) / 2,
                (prev_position.y() + curr_position.y()) / 2,
            ]
            painter.setPen(QPen(Qt.black, 4))
            painter.drawText(
                int(text_vector[0] - 7), int(text_vector[1] + 7), domain.name
            )
